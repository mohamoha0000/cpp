...
using namespace std;
vector<double> loadImageAsInputVector(const string& filename, int target_width = 28, int target_height = 28) {
    ...
    // Check size (optional, or you could resize manually later)
    if (width != target_width || height != target_height) {
        cerr << "Image size must be " << target_width << "x" << target_height << endl;
    ...
    for (int i = 0; i < width * height; ++i) {
        input.push_back(data[i] / 255.0); // normalize to [0.0, 1.0]
    }
...
int main() {
    ...
    Player moha1(hidden, 784); // 28x28

    vector<double> input = loadImageAsInputVector("IMG/test.png");
    cout << endl;
    cin>>n;
